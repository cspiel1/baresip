...
struct ausrc_st {
	...
	size_t sampv_sz;                /**< Size of sample buffer           */
		...
		uint8_t *channels, int *mode, int *extra_headers)
{
   ...
   header = speex_packet_to_header((char*)ogg->op.packet, (int)ogg->op.bytes);
   if (!header) {
...
static int process_packets(struct ausrc_st *st)
{
	...
	if (!st->eof) {
		...
		ogg->serialno = (int)ogg->os.serialno;
		/* Select codec. */
...
static int push_thread(void *arg)
{
	...
	while (st->run) {
		...
		if ( (ts > now + ptime / 4 ||
			...
			sys_msleep((unsigned int)(ts - now));
			continue;
		}

#if 1
		if (now > ts + 100) {
			warning("aufile: cpu lagging behind (%u ms)\n",
				(unsigned int)(now - ts));
		}
#endif
