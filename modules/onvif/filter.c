...
{
	bevent_app_emit(UA_EVENT_CUSTOM, NULL, "%s:%s:%s", obj, ev, detail);
}
	...
	struct call *call, const char *prm, void *arg)
{
	...
	mixer->incodec = audio_codec(call_audio(call), false);
	if (mixer->incodec)
	...
	const struct rtp_header *hdr, struct mbuf *mb, size_t wsampc)
{
	...
	if (aresamp->resamp.ratio) {
		...
		if (!aresamp->sampvre) {
			size_t sz = new_sampc * 2*aufmt_sample_size(fs->fmt);
			aresamp->sampvre = mem_zalloc(sz, NULL);
			if (!aresamp->sampvre) {
				err = ENOMEM;
				return err;
			}
